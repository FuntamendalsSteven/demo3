...
Furthermore, we only keep the columns for country name, variable, and region, thus:
```{r}
maternalmortality_clean <- subset(maternalmortality_clean, select = c(name, deathsper100000livebirths, region))
str(maternalmortality_clean) # view the internal structure of maternalmortality for verification purposes
```
...
Columns name and region are textual columns, and are recognized correctly as character (chr) types by R.
...
Let's move onto the region textual column.
```{r}
...
Furthermore, we only keep the columns for country name, variable, and region, thus:
```{r}
infantmortality_clean <- subset(infantmortality_clean, select = c(name, deathsper1000livebirths, region))
str(infantmortality_clean) # view the internal structure of infantmortality for verification purposes
```
...
Furthermore, we only keep the columns for country name, variable, and region, thus:
```{r}
lifeexpectancy_clean <- subset(lifeexpectancy_clean, select = c(name, years, region))
str(lifeexpectancy_clean) # view the internal structure of infantmortality for verification purposes
```
...
```
The output is 'character(0)' so we know that all entries in the region textual column contain valid characters. We have completed cleaning and formatting the data.

# Saving

We may now save the data objects into RDS and CSV files.
```{r}

nowClean1=maternalmortality_clean[,]
nowClean2=infantmortality_clean[,]
nowClean3=lifeexpectancy_clean[,]
...
write.csv(nowClean1,file.path(folder,"maternalmortality.csv"))
write.csv(nowClean2,file.path(folder,"infantmortality.csv"))
write.csv(nowClean3,file.path(folder,"lifeexpectancy.csv"))

saveRDS(maternalmortality_clean, "Maternal mortality ratio.rds")
saveRDS(infantmortality_clean, "Infant mortality rate.rds")
saveRDS(lifeexpectancy_clean, "Life expectancy at birth.rds")
```

# Verify

```{r}
data <- read.csv("dataCleaned/maternalmortality.csv")
str(data)

data <- read.csv("dataCleaned/infantmortality.csv")
str(data)

data <- read.csv("dataCleaned/lifeexpectancy.csv")
str(data)

data <- readRDS("Maternal mortality ratio.rds")
str(data)

data <- readRDS("Infant mortality rate.rds")
str(data)

data <- readRDS("Life expectancy at birth.rds")
str(data)
